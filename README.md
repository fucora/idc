# 项目简介
本项目是对分布式任务调度框架Quartz的二次开发，Quartz能进行高效的定时任务调度，
但其只能调度单节点任务、不支持工作流特性、无法保证任务间的依赖关系，
即无法完成按照任务间的依赖关系进行自动调度。本项目为简化业务人员工作量也为弥补上述不足而研发。
...
## [2.2.0 doing](http://zentao.iwellmass.com:8090/zentao/task-view-611.html)
- 完成调度计划间依赖
- 支持暂停单节点
