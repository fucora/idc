...
public class IDCScheduler {
	...
	public Task getJob(String name) {
		return taskRepository.findByName(name).orElseThrow(() -> new AppException("未找到调度计划"));
	}

	public void schedule(TaskVO vo) {
		Task task = new Task(vo.getName(), vo.getTaskId(), vo.getTaskGroup());
		...
		// 调度
		try {
			Trigger trigger = vo.buildTrigger(task.getName(), Task.GROUP_PRIMARY);
	...
	public void reschedule(String name, ReTaskVO reVO) {
		...
		try {
			Trigger trigger = reVO.buildTrigger(job.getName(), Task.GROUP_PRIMARY);
